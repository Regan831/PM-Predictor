...
def load_mobile_sensors():
    ...
    for date in constants.dates:
        if date in ['20180705-20180706', '20180706-20180707']:
            sensor= 'XXM008'
        filename = "data/raw/personal/{}/{}_{}.csv".format(date, sensor, date.split("-")[0])
        df = pd.read_csv(filename,index_col=None)
        list_.append(df)
...
def train(all_static_data, mobile_sensor_data, start_time, end_time, par_grid, window, timeint_on_first_window=0, verbose=False):
